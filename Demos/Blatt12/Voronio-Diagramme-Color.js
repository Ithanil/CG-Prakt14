"use strict";
	...
	var gl, canvas, zoom, phi = 0,
		theta = 0,
		aspectratio,
		PMat, MVMat, PMatLoc, MVMatLoc, NMatLoc, colorLoc,
	...
	//Make list of colors for the cones
	var colorList = new Array(); // Random colors for the cones
	for (var i=0;i<S.length;i++) colorList.push(Math.random());
	...
	function webGLStart()
	{
		...
		//location of color for fragment shader
		colorLoc = gl.getUniformLocation(shaderProgram, "uFragColor");
	...
	**********************************/
	...
	function draw_cone(p, b, slices,red,green,blue) // p is position of cone peak r,g,b is color of the cone must be float
	{
		...
		gl.uniform4f(colorLoc,red,green,blue,1.0); //write color if cone in colorLoc for f shaderProgram
	...
	function drawScene() //This is the Line you can allwaysF see
	{
		...
		gl.uniform4f(colorLoc,0.0,1.0,0.0,1.0); //write color if cone in colorLoc for f shaderProgram default will be used for all points
		...
		for (var i=0; i<S.length; i=i+3){
			...
			P[1] = S[i+1];
			P[2] = S[i+2];
			draw_cone(P,Size,50,red,green,blue);
		}
